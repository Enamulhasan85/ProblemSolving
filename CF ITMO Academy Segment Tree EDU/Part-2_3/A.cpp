...
struct segtree{
    ...
    void push(ll i, ll l, ll r){
        ...
        tree[i] = (r-l+1)*lazy[i];
        pre[i] = max(0LL, tree[i]);
        suf[i] = max(0LL, tree[i]);
        mx[i] = max(0LL, tree[i]);
        //tree[i] %= mod;
        lazy[i] = 1e18;
    }
    ...
    void combineind(ll i, ll i1, ll i2){
        ...
        pre[i] = max(pre[i1], tree[i1]+pre[i2]);
        suf[i] = max(suf[i2], tree[i2]+suf[i1]);
        mx[i] = max(mx[i1], mx[i2]);
        mx[i] = max(mx[i], suf[i1]+pre[i2]);
        return ;
    }
